...
function isWeekendCard(dateString) {
...
async function updateCards() {
    try {
        ...
        const cards = [
            {
                ...
                reverse: false,
                date: data.date // Assume date is available in card data
            },
            {
        ...
        cards.forEach(card => {
            ...
            const weekendClass = isWeekendCard(card.date) ? 'weekend-card' : '';
            const cardElement = document.createElement('div');
            cardElement.className = `card p-4 bg-white rounded-lg shadow-lg flex flex-col items-center justify-between ${weekendClass}`;
